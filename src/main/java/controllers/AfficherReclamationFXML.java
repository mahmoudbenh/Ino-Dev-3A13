...
public class AfficherReclamationFXML implements Initializable {
    ...
    void afficher() throws SQLException {
        ...
        reclamations = (ArrayList<reclamation>) srr.selectAll1();
...
//////////////////////////////
    ...
    //////////////////
    @FXML
    void showNonTraiteeCount(ActionEvent event) {
        try {
            ServiceReclamation sr = new ServiceReclamation();
            int nonTraiteeCount = sr.getNonTraiteeReclamationsCount();
            showNonTraiteeCountNotification(nonTraiteeCount);
        } catch (SQLException e) {
            // Gérez l'exception, par exemple, en affichant une boîte de dialogue d'erreur
            e.printStackTrace();
        }
    }
    private void showNonTraiteeCountNotification(int count) {
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Nombre de Réclamations Non Traitées");
        alert.setHeaderText(null);
        alert.setContentText("Le nombre de réclamations non traitées est : " + count);
        alert.showAndWait();
    }
