...
public class ServiceReclamation implements CRUDR<reclamation> {
    ...
    public List<reclamation> selectAll1() throws SQLException{
...

        {
            ...
            r.setTitre_reclamation(rs.getString(3));
            r.setDate_reclamation(rs.getDate(4).toLocalDate());
            r.setHeure(rs.getTime(5).toLocalTime());
            r.setType_reclamation(rs.getString(6));
            r.setStatut_reclamation(rs.getString(7));
    ...
    public int getNonTraiteeReclamationsCount() throws SQLException {
        String req = "SELECT COUNT(*) AS count FROM reclamation WHERE statut_reclamation = 'Non traitée'";
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(req);
        ...
        return 0; // Si aucune réclamation non traitée n'est trouvée
    }
